...
list_type* create_list(int N)
{
	...
	int return_value = regcomp(&regex, "^[A-ZА-Я0-9](([A-ZА-Яa-zа-я0-9_])|([,?!\\. ][^\\.,?! ]))+", REG_EXTENDED);
	...
	for(int i = 0; i < N; i++){
		...
		return_value = regexec(&regex, human, 0, NULL, 0);
		
		if (return_value == 0)
		{
			printf("\n[%d]Pattern for HUMAN found\n",count);
		}else{
			printf("[%d]Pattern for HUMAN not found\n",count);
		}
		...
		return_value = regexec(&regex, gender, 0, NULL, 0);
		
		if (return_value == 0)
		{
			printf("[%d]Pattern for GENDER found\n",count);
		}else{
			printf("[%d]Pattern for GENDER not found\n",count);
		}
		...
		return_value = regexec(&regex, node->myfighter.name, 0, NULL, 0);
		
		if (return_value == 0)
		{
			printf("[%d]Pattern for NAME found\n",count);
		}else{
			printf("[%d]Pattern for NAME not found\n",count);
		}
		...
		return_value = regexec(&regex, bonus, 0, NULL, 0);
		
		if (return_value == 0)
		{
			printf("[%d]Pattern for BONUS found\n",count);
		}else{
			printf("[%d]Pattern for BONUS not found\n",count);
		}
		...
		return_value = regexec(&regex, clancolor, 0, NULL, 0);
		
		if (return_value == 0)
		{
			printf("[%d]Pattern for CLAN found\n",count);
		}else{
			printf("[%d]Pattern for CLAN not found\n",count);
		}
